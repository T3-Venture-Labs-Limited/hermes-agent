...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        # ── Myah: follow-up suggestion generation ────────────────────────
        # Fired after every assistant turn to generate clickable follow-up
        # prompts shown in the Myah web UI. Mirrors upstream's
        # title_generation structure. `provider: auto` inherits whichever
        # provider the user has configured via UserLLMKeys, so users who
        # bring their own Anthropic/OpenAI/OpenRouter key see their own
        # model used for this background task (no silent cross-billing).
