...
class TestStreamingApiCall:
    ...
    def test_tool_call_accumulation(self, agent):
        # Upstream uses assignment (not +=) for function names because the
        # OpenAI spec treats them as atomic identifiers delivered complete in
        # the first name-bearing chunk.  With assignment the final name is the
        # last non-empty chunk's value; arguments still use +=.
        chunks = [
        ...
        assert len(tc) == 1
        # Name = last non-empty name chunk (assignment, not concatenation)
        assert tc[0].function.name == "search"
        assert tc[0].function.arguments == '{"q":"test"}'
        assert tc[0].id == "call_1"

    def test_tool_name_not_duplicated_when_provider_repeats_it(self, agent):
        # Some providers (e.g. MiniMax M2.7 via NVIDIA NIM) resend the full
        # function name in every chunk.  Using += would produce "web_searchweb_search".
        # Using assignment is immune to this.
        chunks = [
            _make_chunk(tool_calls=[_make_tc_delta(0, "call_1", "web_search", '{"q":')]),
            _make_chunk(tool_calls=[_make_tc_delta(0, None, "web_search", '"test"}')]),
            _make_chunk(finish_reason="tool_calls"),
        ]
        agent.client.chat.completions.create.return_value = iter(chunks)
