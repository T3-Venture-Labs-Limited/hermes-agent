...
def _get_current_otel_trace_id() -> str:
    try:
...
class AIAgent:
    ...
    def _start_llm_span(
        self, *, model: Optional[str] = None, iteration: Optional[int] = None
    ):
        tracer = get_tracer()
        if tracer is None:
            yield None
        ...
        with tracer.start_as_current_span("llm.call") as span:
            try:
                span.set_attribute("model", str(model or self.model or ""))
                span.set_attribute("provider", str(self.provider or ""))
    ...
    @staticmethod
    def _mark_span_exception(span: Any, exc: Exception) -> None:
        if span is None:
            return
        try:
        ...
        def _call():
            ...
            try:
                with self._start_llm_span(
                    iteration=getattr(self, "_current_api_iteration", None)
                ) as span:
                ...
                self._mark_span_exception(span, e)
                result["error"] = e
            finally:
    ...
    def run_conversation(
        ...
        while (
            ...
            while retry_count < max_retries:
                try:
                    self._current_api_iteration = api_call_count
