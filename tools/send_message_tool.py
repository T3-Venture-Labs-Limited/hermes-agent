...
async def _send_myah(pconfig, chat_id: str, message: str):
    ...
    The Myah adapter keeps a mapping from chat_id to active SSE streams.
    If the user's chat is connected, we push a ``message.delta`` event into
    the stream so the frontend renders it in real-time.  If not connected
    (user closed the browser), the message is effectively dropped — there's
    no background queue or push notification for Myah yet.
    """
    try:
