    ...
    def _get_or_create_honcho_manager(self, chat_id: str):
        ...
        try:
            ...
            if not hcfg.enabled or not (hcfg.api_key or hcfg.base_url):
                logger.debug(
                    "Honcho disabled or not configured — skipping manager pool for %s",
                    chat_id,
                )
                return None, hcfg
        ...
        try:
                        ...
                        while True:
                            try:
                                ...
                                if (
                                    ...
                                    and delta.get("_event_type") == "tool.event"
                                ):
                                    event_data = {
                                        k: v
            ...
            if isinstance(result, dict):
                ...
                if _render_tools:
                    ...
                    await response.write(
                        f"data: {json.dumps(_tc_chunk, default=str)}\n\n".encode()
                    )
            # ─────────────────────────────────────────────────────────────────
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            try:
                ...
                _conv_fn = _lf_obs(as_type="agent", name="hermes-conversation")(
        ...
        with _lf_ctx:
            ...
            if otel_trace_id:
                logger.debug(
                    "Agent conversation starting",
                    extra={"otel_trace_id": otel_trace_id},
                )
