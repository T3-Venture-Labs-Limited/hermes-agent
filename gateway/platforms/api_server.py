...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        reasoning_callback=None,  # Myah: reasoning token streaming
        model_override: Optional[str] = None,  # Myah: per-request model override
    ) -> Any:
        """
        ...
        model = model_override or _resolve_gateway_model()  # Myah: per-request model override
        ...
        agent = AIAgent(
            ...
            **runtime_kwargs,
            ...
            reasoning_callback=reasoning_callback,  # Myah: reasoning token streaming
    ...
    _UPDATE_ALLOWED_FIELDS = {"name", "schedule", "prompt", "deliver", "skills", "skill", "repeat", "enabled", "chat_id"}  # Myah: chat_id for cron job delivery
    ...
    def _make_run_event_callback(self, run_id: str, loop: "asyncio.AbstractEventLoop"):
        ...
        # ── Myah: extended tool event fields (call_id, args, result) ──
        _call_ids: Dict[str, str] = {}  # call_id -> tool name
        # ────────────────────────────────────────────────────────────

        def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
            ts = time.time()
            call_id = kwargs.get("call_id", "")  # Myah: track call_id
            if event_type == "tool.started":
                # ── Myah: track call_id for tool correlation ──────────
                if call_id:
                ...
                _push({
                    ...
                    "tool": tool_name,
                    "preview": preview,
                    "call_id": call_id,  # Myah: tool correlation
                    "args": str(args)[:500] if args else "",  # Myah: tool args
                })
            elif event_type == "tool.completed":
                result = kwargs.get("result")  # Myah: tool result
                _push({
                    ...
                    "call_id": call_id,  # Myah: tool correlation
                    "args": str(args)[:500] if args else "",  # Myah: tool args
                    "result": str(result)[:1000] if result else "",  # Myah: tool result
                })
    ...
    # ── Myah: approval confirmation endpoint ─────────────────────────
    ...
    async def connect(self) -> bool:
        ...
        try:
            mws = [mw for mw in (cors_middleware, body_limit_middleware, security_headers_middleware, sentry_trace_middleware) if mw is not None]  # Myah: sentry_trace_middleware
            ...
            self._app.router.add_post("/v1/runs/{run_id}/confirm", self._handle_run_confirm)  # Myah: approval confirmation
