...
class BasePlatformAdapter(ABC):
    ...
    # ── Myah: cron-delivery metadata hook (Tier 2B Task 2B.4 / Phase 4f) ──────
    # Polymorphic alternative to the legacy ``if platform_name == "myah"``
    # branch in cron/scheduler.py. Adapters that need to enrich metadata for
    # cron delivery (e.g. MyahAdapter for the offline-webhook fallback that
    # reconstructs the run-complete payload) override this. The default just
    # returns ``base_metadata`` unchanged. cron.scheduler._deliver_result
    # calls this polymorphically, so no per-platform branches are needed in
    # the scheduler. Same diff queued as upstream PR U-CRON.
