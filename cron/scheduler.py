...
def _deliver_result(
    ...
    adapters=None,
    loop=None,
    status_hint: str = "ok",  # Myah: forwarded to build_delivery_metadata polymorphic hook
) -> Optional[str]:
    """
    ...
    for target in targets:
        ...
        if runtime_adapter is not None and loop is not None and getattr(loop, "is_running", lambda: False)():
            # ── Myah: polymorphic cron-delivery metadata (Tier 2B Task 2B.4) ──
    ...
    try:
        ...
        def _process_job(job: dict) -> bool:
            ...
            try:
                ...
                if should_deliver:
                    try:
                        # ── Myah: forward run status to delivery metadata hook ────
